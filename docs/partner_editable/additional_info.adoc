...
Every new AWS account comes with a default VPC with public subnets in each Availability Zone. It is recommended that you delete the default VPC and only deploy resources into the Production, Management, and Development VPCs that the Biotech Blueprint Quick Start creates. If you have already deployed resources into the default VPC before launching the Quick Start, it is recommended that you migrate these resources to the Biotech Blueprint VPCs and then delete the default VPC. Removing the default VPC will ensure that a user does not mistakenly launch a resource into one of its exposed public subnets.
...
For example, the Operational Best Practices for NIST-CSF Conformance pack comes with 93 rules. One of which is `encrypted-volumes-conformance-pack`, which checks whether EBS volumes that are in an attached state are encrypted. 
