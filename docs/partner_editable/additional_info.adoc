...
== Optional DNS setup
The Quick Start sets up a private DNS with `.corp` as the apex domain using https://console.aws.amazon.com/route53/v2/home#Dashboard[Amazon Route 53 in your account^]. You can create A or CNAME records to private applications you deploy using the Amazon Route 53 console. 

For example, suppose you launch a development server with private IP `10.60.0.198`. Using the Amazon Route 53 console, you can create an A record to the address in the `.corp` hosted zone with a simpler name such as `abc-machine-corp`. Then, resources in all three VPCs and clients connected to the client VPN endpoint can resolve `abc-machine.corp` from a browser, terminal, or API call.
