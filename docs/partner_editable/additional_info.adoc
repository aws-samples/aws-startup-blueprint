...
+
NOTE: The other three files are the CA chain and server key/cert. You will need those if you want to create additional client certificates later on.
...
[start=6]
. Save your changes to `downloaded-client-config.ovpn`. You can open or import the configuration file with an OpenVPN client. 
